<template slot="header">
  <bk-navigation header-title="nav.id" side-title="nav.title" :default-open="true" navigation-type="top-bottom"
      need-menu=true>
      <div class="searchNav">
  ...
  <div class="monitor-navigation-header">
    <ol class="header-nav">
      <bk-popover v-for="(item,index) in header.list" :key="item.id" theme="light navigation-message" :arrow="false"
        ...
        <li v-show="item.show" class="header-nav-item" :class="{ 'item-active': index === header.active }" @click="handleChangeNav(item.id)">
          {{item.name}}
        </li>
      </bk-popover>
    </ol>
    ...
    <div class="header-help" :class="{ 'is-left': false }">
    ...
    <bk-popover theme="light navigation-message" :arrow="false" offset="-20, 10" placement="bottom-start"
      ...
      <div class="header-user" :class="{ 'is-left': false }">
        Charles
